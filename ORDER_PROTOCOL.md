...
| MCP 정본 | 사용자 `~/.codex/config.toml` | Codex Order가 대화형 CLI와 동일하게 상속하는 MCP 설정 |
...
**codex**: Node로 `codex.js`를 직접 실행한다. 사용자 `~/.codex/config.toml`을 상속하여 등록된 MCP 전체를 대화형 CLI와 동일하게 로드한다. 동시에 `approval_policy="never"`와 `windows.sandbox="elevated"`는 최고 우선순위 CLI `-c`로 재강제하고, `workspace-write`와 `--add-dir COMMON_ROOT`로 쓰기 범위를 제한한다. dispatcher는 실행 직전 MCP 이름 목록을 stdout 및 `logs/dispatcher/mcp-audit.log`에 기록하며, 보수적으로 분류한 write 가능 MCP는 차단하지 않고 경고한다. 설정값·토큰·명령 인자는 로그에 남기지 않는다. Order의 `도구:` 행은 선택적 로딩 필터가 아니다.
...
- **MCP 로딩은 실행자별 비대칭** — Codex는 사용자 `~/.codex/config.toml`의 등록 MCP 전체를 상속한다. Claude는 로컬 설정을 이어받되 `.claude/settings.json` 허용목록의 적용을 받는다. `도구:` 행을 선택적 MCP 로딩으로 변환하는 기능은 없다.
